...
import { useGastosRecurrentesOrdenados } from "../hooks/useGastosRecurrentesOrdenados";
...
import { useEstadoGastosRecurrentes } from "../hooks/useEstadoGastosRecurrentes";
...
export const GastosPage = () => {
    ...
    // ---------------
    // Section Tabla: acciones para el filtrado de la tabla de gastos por categoria
    const [activaCalculadora, setActivaCalculadora] = useState(false);
    const [selectCategoria, setSelectCategoria] = useState<CategoriaGastoResponse>();
    
    const gastosAMostrar = !activaCalculadora
        ? gastosFiltrados.filter(
            gasto => !selectCategoria || gasto.categoriaGastoId === selectCategoria.id
        )
        : gastosFiltrados;
    ...
    const sumarGasto = (categoria: CategoriaGastoResponse) => {
    ...
    const handleCategoriaClick = (categoria: CategoriaGastoResponse) => {
        setSelectCategoria(categoria);
        activaCalculadora && sumarGasto(categoria);
    };
    
    console.log("------fitlrado:",activaCalculadora)
            ...
            <section className="flex flex-col justify-center min-h-0 gap-1">
                <div className="flex items-center justify-between text-sm">
                    <div className="flex-[2] flex items-center justify-end gap-1">
                        <BlurOnOutlinedIcon
                            sx={{
                                fontSize: !activaCalculadora ? 15 : 20, 
                                color: !activaCalculadora ? theme.colors[300] : theme.colors[700],
                            }} 
                        /> 
                    ...
                    <button 
                        onClick={() => {
                            {activaCalculadora ? setActivaCalculadora(false) : setActivaCalculadora(true)};
                        }}
                    ...
                    <div className="flex-[2] flex items-center justify-start gap-1">
                        ...
                        <BlurOnOutlinedIcon
                            sx={{
                                fontSize: activaCalculadora ? 20 : 15,  
                                color: activaCalculadora ? theme.colors[700] : theme.colors[300],
                            }} 
                        /> 
                    </div>
                </div>
                <div>
                    ...
                    > {activaCalculadora ? (selectCategoria ? `• ${selectCategoria?.nombre.toUpperCase()} •` : "• Seleccionar categoria •") : ""} </h1>
                </div>
                <TablaMoney
